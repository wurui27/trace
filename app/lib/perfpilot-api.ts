...
import {
  parseAnalysisListResponse as parseAnalysisListContract,
  parseAnalysisResponse as parseAnalysisContract,
...
function analysisReportResponse(value: unknown): AnalysisReport {
  ...
  if (value.schema_version === "1.2") {
    ...
    if (
      ...
      !validOriginalMetadata(value.smartperfetto_original)
    ) {
      throw new PerfPilotApiError("invalid_api_response", "服务返回内容无效", false, null);
    }
    const completed =
...
function analysisResponse(value: unknown): AnalysisResponse {
  return parseAnalysisContract(
    value,
    () => new PerfPilotApiError("invalid_api_response", "服务返回内容无效", false, null),
...
function analysisListResponse(
  ...
  return parseAnalysisListContract(
    value,
    context,
    () => new PerfPilotApiError("invalid_api_response", "服务返回内容无效", false, null),
  ) as unknown as AnalysisListResponse;
}
