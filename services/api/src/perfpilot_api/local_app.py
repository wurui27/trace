...
from perfpilot_api.local_analysis_projection import (
    LocalAIRound as _LocalAIRound,
    LocalAnalysisView,
    PersistedLocalEvidenceError as _PersistedLocalEvidenceError,
    compose_local_report as _compose_local_report,
    default_ai_rounds as _default_ai_rounds,
    from_persisted_document,
    local_source_code_document as _local_source_code_document,  # noqa: F401
    prepared_from_persisted_documents as _prepared_from_persisted_documents,
    restore_ai_rounds as _restore_ai_rounds,
    restore_evidence_manifest as _restore_evidence_manifest,
    source_binding_document as _source_binding_document,
    source_code_analysis_document as _source_code_analysis_document,
    source_code_analysis_unavailable_document as _source_code_analysis_unavailable_document,
    source_metadata as _source_metadata,
    to_persisted_document,
    to_public_document,
    validate_persisted_state_shape as _validate_persisted_state_shape,
)
...
from perfpilot_api.local_stage_execution import (
    NormalizedLocalResult as _NormalizedLocalResult,
    PreparedLocalReport as _PreparedLocalReport,
    execute_report_stage,
    normalize_local_smartperfetto_result as _normalize_local_smartperfetto_result,
    prepare_local_report as _prepare_local_report,
    remote_capture_question as _remote_capture_question,
    trace_upload_question as _trace_upload_question,
)
...
class _LocalRuntime:
    ...
    async def start(self) -> None:
                ...
                if manifest_document is not None:
                    ...
                    try:
                        if analysis.device_agent_id is None:
                            raise ValueError
                        access, manifest = restore_remote_capture(
                            team_id=analysis.team_id,
                            analysis_id=analysis.analysis_id,
                            agent_id=analysis.device_agent_id,
                            capture_test_type=(
                                analysis.capture_configuration.get("test_type")
                                if analysis.capture_configuration is not None
                                else None
                            ),
        ...
        for analysis in pending_script_publications:
            await self.agent_tasks.enqueue(script_device_definition(analysis))
    ...
    async def _publish_remote_device(
        ...
        definition = remote_device_definition(analysis, upload)
