...
class FakeAnalysisRepository:
    def __init__(self, events: list[str]) -> None:
        ...
        self.active_analysis_ids: tuple[UUID, ...] = ()
        self.active_list_calls: list[dict[str, object]] = []
    ...
    async def list_active_analysis_ids(self, **kwargs: object) -> tuple[UUID, ...]:
        self.events.append("list_active_analysis_ids")
        self.active_list_calls.append(kwargs)
        return self.active_analysis_ids
...
@pytest.mark.asyncio
async def test_list_active_analyses_preserves_repository_newest_first_order() -> None:
    events: list[str] = []
    repository = FakeAnalysisRepository(events)
    newer_id = UUID("30000000-0000-4000-8000-000000000010")
    older_id = UUID("30000000-0000-4000-8000-000000000009")
    repository.active_analysis_ids = (newer_id, older_id)
    repository.views = {
        newer_id: replace(
            _analysis_view(),
            analysis_id=newer_id,
    ...
    views = await service.list_active_analyses(team_id=TEAM_ID, limit=2)

    assert [view.analysis_id for view in views] == [newer_id, older_id]
    assert repository.active_list_calls == [{"team_id": TEAM_ID, "limit": 2}]
    assert events == [
        "list_active_analysis_ids",
        "load_view",
        "load_view",
    ]
...
@pytest.mark.parametrize("limit", (0, 21))
@pytest.mark.asyncio
async def test_list_active_analyses_rejects_out_of_range_limit(limit: int) -> None:
    from perfpilot_api.services.analyses import AnalysisInvalidRequestError
    ...
    with pytest.raises(AnalysisInvalidRequestError):
        await service.list_active_analyses(team_id=TEAM_ID, limit=limit)
